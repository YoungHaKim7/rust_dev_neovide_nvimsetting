...
local custom_attach = function(_,bufnr)
    ...
    if vim.fn.has('macunix') == 1 and vim.fn.exists('neovide') == 1 or vim.fn.has('gui_running') == 1 then
        nnoremap([[<C-'>]], [[:RustToggleInlayHints<CR>]], true)
        nnoremap("<C-x>", [[:lua require'dap'.toggle_breakpoint()<CR>]], true)
        nnoremap([[<C-\\>]], [[:RustParentModule<CR>]], true)
        nnoremap([[<C-[>]], [[:RustMoveItemDown<CR>]], true)
        nnoremap([[<C-]>]], [[:RustMoveItemUp<CR>]], true)
        nnoremap([[<C-F1>]], [[:lua require'dap'.continue()<CR>]], true)
        nnoremap([[<C-F2>]], [[:lua require'dap'.step_over()<CR>]], true)
        nnoremap([[<C-F3>]], [[:lua require'dap'.step_into()<CR>]], true)
        nnoremap([[<C-F4>]], [[:lua require'dap'.step_out()<CR>]], true)
        nnoremap([[<C-F5>]], [[:lua require'dap'.close()<CR>]], true)
        vim.keymap.set("n", "<Leader>a", rt.code_action_group.code_action_group, { buffer = bufnr })
        vim.keymap.set("n", "<Leader>v", rt.hover_actions.hover_actions, { buffer = bufnr })
        nnoremap([[<C-`>]], [[:lua require'dapui'.toggle()<CR>]], true)
        nnoremap([[<C-F8>]], [[:lua require'dap'.repl.close()<CR>]], true)

        nnoremap([[<C-c>]], [[:RustPlay<CR>]], true)
    else
